...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       // Toast.makeText(context = this, text= "io penso positivo", Toast.LENGTH_LONG).show()

    }
    fun getRandomMessage(): String{
        pos = Random.nextInt(database.size)
        return database[pos]
    }
    ...
    fun prev(view : View){
        pos--
        if (pos < 0) pos = database.size - 1
        textView.text = database[pos]
    }

    fun rand( view: View){
        textView.text = getRandomMessage()
    }
    fun next( view: View){
        pos++
        if (pos >database.size-1) pos = 0
        textView.text = database[pos]
    }
    fun edit(view : View){
        if (busqueda.text.isNotEmpty())
            if((busqueda.text).toString().toInt()>database.size-1) {
                Toast.makeText(this, "el numero es mayor al arreglo", Toast.LENGTH_SHORT).show()
            } else{
                if((busqueda.text).toString().toInt()<0) {
                    Toast.makeText(this, "el numero es menor al arreglo", Toast.LENGTH_SHORT).show()
                }else{
                    pos = busqueda.text.toString().toInt()
                    textView.text = database[pos]
                }
            }
